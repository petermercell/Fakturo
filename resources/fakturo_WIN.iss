...
;
; This file is UTF-8 **with a byte order mark**, and must stay that way. Inno
; Setup reads a BOM-less script in the system ANSI codepage, so "Odinstalovat"
; below - and the em dashes - would reach the wizard as mojibake on a Slovak
; program. Same root cause as the two encoding bugs in WINDOWS.md: rc.exe did
; it to the version block, PowerShell 5.1 did it to make-windows.ps1. Unlike
; those two there is no #pragma to reach for here; the BOM is the mechanism.
;
